...
#source("code/helper/ch00_install_libraries.R")
# Import libraries
library(haven)
library(glmnet)
library(purrr)
...
data <- data %>%
  group_by(comp_id) %>%
  mutate(growth_two_years =  (Lag(sales_mil, -2) - sales_mil) / sales_mil ) %>%
  ungroup()
# IMPORTANT: it is important not to use growth_two_years as a predictor variable!!!!!

#fast growing if growth_two_years > 50%

data  <- data %>%
  mutate(fast_growing = growth_two_years > 0.5 & !is.na(growth_two_years) %>%
           as.numeric(.))
...
# replace NA w -100 if a firm exits next year
data <- data %>%
  mutate(growth_two_years = ifelse(is.na(growth_two_years), -1, growth_two_years))
...
Hmisc::describe(data$fast_growing)
write_csv(data,paste0(data_in,"work5.csv"))

###########################################################
# Feature engineering
###########################################################

data <- read_csv(paste0(data_in,"work5.csv"))
...
ggplot(data = data, aes(x=data$d1_sales_mil_log, y=as.numeric(fast_growing))) +
  ...
  labs(x = "delta sales",y = "fast growing") +
  theme_bg()
...
d1sale_1<-ggplot(data = data, aes(x=ceo_age, y=as.numeric(fast_growing))) +
...
d1sale_2<-ggplot(data = data, aes(x=d1_sales_mil_log_mod, y=as.numeric(fast_growing))) +
  geom_point(size=0.1,  shape=20, stroke=2, fill=color[2], color=color[2]) +
  geom_smooth(method="loess", se=F, colour=color[1], size=1.5, span=0.9) +
  labs(x = "Growth rate (Diff of ln sales)",y = "fast_growing") +
  theme_bg() +
